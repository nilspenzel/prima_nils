...
export const actions = {
	bookItineraryWithOdm: async ({ request, locals }): Promise<{ msg: Msg }> => {
		...
		let journey: SignedItinerary | undefined = undefined;
		try {
			journey = JSON.parse(json) as SignedItinerary;
		} catch (_) {
			console.log(
		...
		const firstOdmIndex = journey!.legs.findIndex((l: Leg) => l.mode === 'ODM');
		if (firstOdmIndex === -1) {
			console.log(
		...
		const firstOdm = journey!.legs[firstOdmIndex];
		const lastOdmIndex = journey!.legs.findLastIndex((l: Leg) => l.mode === 'ODM');
		const lastOdm = journey!.legs[lastOdmIndex];
		if (!journey.signature1) {
			console.log(
		...
		const connection1 = expectedConnectionFromLeg(firstOdm, journey.signature1);
		const connection2 =
			firstOdmIndex === lastOdmIndex
				? null
				: expectedConnectionFromLeg(lastOdm, journey.signature2);
