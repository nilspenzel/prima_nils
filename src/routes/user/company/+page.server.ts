...
import { te, ttt, ttt2, ttt3, ttt4 } from '$lib/testy.js';
import { getTours } from '$lib/testHelpers.js';

export const load: PageServerLoad = async (event: RequestEvent) => {
	//await ttt3(true, event);
	//await te(event);
	//await ttt4();
	const capacities = {
		wheelchairs: 0,
	...
	const exp1 = {
		start: {
			coordinates: new Coordinates(51.50795702646258, 14.644736712069601),
			address: ''
		},
		target: {
	...
	const exp2 = {
		start: {
			coordinates: new Coordinates(51.50673200918544, 14.626762040891634),
			address: ''
		},
		target: {
			coordinates: new Coordinates(1, 1),
	...
	//await booking(event, exp1.start, exp2.start,true,new Date('2026-09-30T08:47:00'),1,0,0,0);
	//insertRequest(c1, capacities, exp1, event.locals.user!.id, [], []);
	//const aa = await db.selectFrom('event').selectAll().execute();
	//const bab = aa[0].communicated_time;
	//console.log(bab.getTime());
	//console.log(bab instanceof Date);
