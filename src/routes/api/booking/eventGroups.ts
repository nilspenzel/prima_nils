...
import { isSamePlace } from '$lib/bookingAPI/utils';
...
export const getEventGroupInfo = (
	...
	if (how == InsertHow.NEW_TOUR) {
		...
		newEventGroup = !isSamePlace(comparisonEvent.coordinates, coordinates)
			? uuidv4()
...
const getEventGroupUpdates = (
	...
	for (let i = nextEventIdx; i != events.length; ++i) {
		if (nextTour != events[i].tourId || !isSamePlace(events[i].coordinates, coordinates)) {
			break;
		}
		updateList.push({
			id: events[i].id,
