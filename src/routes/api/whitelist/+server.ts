...
export async function POST(event: RequestEvent) {
	...
	const busStops: BusStop[] = parameters.busStops;
	...
	const userChosen: Coordinates = parameters.userChosen;
	const startFixed: boolean = parameters.startFixed;
	const required: Capacities = parameters.capacities;
	const companies = await bookingApiQuery(userChosen, required, searchInterval, busStopCoordinates);
	...
	return json(takeBest(takeBest(bothEvaluations, newTourEvaluations), pairEvaluations));
}
