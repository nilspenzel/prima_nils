...
const inX = (m: number) => {
	return new Date(BASE_MS + minutesToMs(m));
};
...
describe('subtract interval arrays test', () => {
	it('overlapping', () => {
		...
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(1);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(3);
		expect((res[1].startTime.getTime() - BASE_MS) / 60000).toBe(8);
		expect((res[1].endTime.getTime() - BASE_MS) / 60000).toBe(9);
	});
	it('touching', () => {
		const a = [new Interval(inX(1), inX(5)), new Interval(inX(7), inX(9))];
		...
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(1);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(5);
	});
	it('minuend contains subtrahend', () => {
		...
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(1);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(5);
		expect((res[1].startTime.getTime() - BASE_MS) / 60000).toBe(9);
		expect((res[1].endTime.getTime() - BASE_MS) / 60000).toBe(10);
	});
	...
	it('minuend contains subtrahend, same startpoint', () => {
		...
		const res = Interval.subtract(a, b);
		expect(res.length).toBe(1);
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(7);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(10);
	});
	it('equal', () => {
	...
	it('2 minuends', () => {
		...
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(1);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(5);
		expect((res[1].startTime.getTime() - BASE_MS) / 60000).toBe(10);
		expect((res[1].endTime.getTime() - BASE_MS) / 60000).toBe(12);
	});
	it('2 subtrahends', () => {
		...
		const res = Interval.subtract(a, b);
		expect(res.length).toBe(1);
		expect((res[0].startTime.getTime() - BASE_MS) / 60000).toBe(4);
		expect((res[0].endTime.getTime() - BASE_MS) / 60000).toBe(5);
	});
});
