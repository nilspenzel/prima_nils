...
describe('IterateAllInsertions tests', () => {
	it('test IterateAllInsertions', async () => {
		...
		oneToManyResults = oneToManyResults.concat(toUserChosenEvent.length === 0 ? [] : (await Promise.all(toUserChosenEvent))[0]);
		oneToManyResults = oneToManyResults.concat(toUserChosenCompany.length === 0 ? [] : (await Promise.all(toUserChosenCompany))[0]);
		oneToManyResults = oneToManyResults.concat(toUserChosenPredEvent.length === 0 ? [] : (await Promise.all(toUserChosenPredEvent))[0]);
		oneToManyResults = oneToManyResults.concat(fromUserChosenEvent.length === 0 ? [] : (await Promise.all(fromUserChosenEvent))[0]);
		oneToManyResults = oneToManyResults.concat(fromUserChosenCompany.length === 0 ? [] : (await Promise.all(fromUserChosenCompany))[0]);
		oneToManyResults = oneToManyResults.concat(fromUserChosenPostEvent.length === 0 ? [] : (await Promise.all(fromUserChosenPostEvent))[0]);
		...
		for(let i=0;i!=busStops.length;++i){
			oneToManyResults = oneToManyResults.concat(toBusStopEvent[i].length===0 ? [] : (await Promise.all(toBusStopEvent[i]))[0]);
			oneToManyResults = oneToManyResults.concat(toBusStopCompany[i].length===0 ? [] : (await Promise.all(toBusStopCompany[i]))[0]);
			oneToManyResults = oneToManyResults.concat(toBusStopPredEvent[i].length===0 ? [] : (await Promise.all(toBusStopPredEvent[i]))[0]);
			oneToManyResults = oneToManyResults.concat(fromBusStopEvent[i].length===0 ? [] : (await Promise.all(fromBusStopEvent[i]))[0]);
			oneToManyResults = oneToManyResults.concat(fromBusStopCompany[i].length===0 ? [] : (await Promise.all(fromBusStopCompany[i]))[0]);
			oneToManyResults = oneToManyResults.concat(fromBusStopPostEvent[i].length===0 ? [] : (await Promise.all(fromBusStopPostEvent[i]))[0]);
