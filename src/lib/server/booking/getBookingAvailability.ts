...
function mergeEventGroups(
	events: (DbEvent & { time: Interval })[]
): (DbEvent & { time: Interval })[] {
	function getMergedEvent(events: (DbEvent & { time: Interval })[]) {
		if (events.length === 1) {
			return events[0];
		}
		const mergedEvent = events[0];
		mergedEvent.isPickup = true;
		mergedEvent.passengers = events.reduce(
			(acc, curr) => (acc += curr.isPickup ? curr.passengers : -curr.passengers),
			0
		);
		mergedEvent.wheelchairs = events.reduce(
			(acc, curr) => (acc += curr.isPickup ? curr.wheelchairs : -curr.wheelchairs),
			0
		);
		mergedEvent.bikes = events.reduce(
			(acc, curr) => (acc += curr.isPickup ? curr.bikes : -curr.bikes),
			0
		);
		mergedEvent.luggage = events.reduce(
			(acc, curr) => (acc += curr.isPickup ? curr.luggage : -curr.luggage),
			0
		);
		return mergedEvent;
	}
	...
	const result: (DbEvent & { time: Interval })[] = [];
...
const createVehicle = (v: DbVehicle, expandedSearchInterval: Interval) => {
	...
	const events = mergeEventGroups(
		tours.flatMap((t) => sortEventsByTime(t.events.map((e) => createEvent(e))))
	);
	return {
		...v,
