...
describe('Concatenation tests', () => {
	it('create tour concetanation, simple append', async () => {
		...
		const connection1: ExpectedConnection = {
			start: { ...inRothenburg2, address: 'start address' },
			target: { ...inHorka1, address: 'target address' },
		...
		const appendConnection: ExpectedConnection = {
			start: { ...inHorka1, address: 'start address' },
			target: { ...inGehege, address: 'target address' },
			startTime: whiteResponse2.direct[0].pickupTime,
			targetTime: whiteResponse2.direct[0].dropoffTime,
			signature: '',
			startFixed: false,
			sendTime: inXMinutes(80)
		};
		...
		expect(isSamePlace(events[1], inHorka1)).toBe(true);
		expect(isSamePlace(events[2], inHorka1)).toBe(true);
		expect(isSamePlace(events[3], inGehege)).toBe(true);
	});
});
