...
export async function up(db) {
    ...
    await db
        ...
        .columns(['lat', 'lng', 'scheduled_time_start', 'scheduled_time_end', 'communicated_time', 'prev_leg_duration', 'next_leg_duration', 'address', 'request', 'is_pickup'])
        .expression(
        db
            .selectFrom('event')
            .select(['lat', 'lng', 'scheduled_time_start', 'scheduled_time_end', 'communicated_time', 'prev_leg_duration', 'next_leg_duration', 'address', 'request', 'is_pickup'])
        )
        .execute();
    ...
    BEGIN
        INSERT INTO event_group (
            lat, lng, scheduled_time_start, scheduled_time_end, communicated_time,
        ...
        VALUES (
            ...
            p_event.prev_leg_duration, p_event.next_leg_duration
        )
    ...
    BEGIN
        INSERT INTO event (
            is_pickup, request, event_group_id, cancelled
        )
        VALUES (
            p_event.is_pickup, p_request_id, p_event_group, FALSE
        );
    END;
    $$ LANGUAGE plpgsql;
    `.execute(db);
    ...
    BEGIN
        ...
        SET
            ...
            communicated_time = p_event.communicated_time
    ...
    BEGIN
    	...
    	IF (
    		...
    		ORDER BY eg.communicated_time ASC
    		LIMIT 1
    	) <= p_now THEN
    		RETURN FALSE;
    	END IF;
